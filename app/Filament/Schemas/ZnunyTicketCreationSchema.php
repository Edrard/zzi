...
{
    public static function schema(?string $draftToken = null): array
    {
        return [
            Section::make()
                ->compact()
                ->schema([
                    ...
                    RichEditor::make('body')
                        ...
                        ->required()
                        ->extraAttributes(['class' => 'znuny-ticket-body-editor'])
                        ->fileAttachmentsDisk('local')
                        ->fileAttachmentsDirectory(function () use ($draftToken) {
                            $userId = auth()->id() ?? 'guest';

                            return "znuny-ticket-inline/{$userId}/{$draftToken}";
                        })
                        ->fileAttachmentsVisibility('private')
                        ->fileAttachmentsAcceptedFileTypes(['image/png', 'image/jpeg', 'image/gif', 'image/webp'])
                        ->preventFileAttachmentPathTampering(true, function (string $file) use ($draftToken) {
                            if (! $draftToken) {
                                return false;
                            }
                            $userId = auth()->id() ?? 'guest';

                            return str_starts_with($file, "znuny-ticket-inline/{$userId}/{$draftToken}/");
                        }),
                ]),
